...
void updateData() {
  ...
  if (currentPage == 0) {
    ...
    if (tdaGain != l_gain || tdaMute != l_mute || blinkUpdate) {
      int fill = map(constrain(tdaGain, 0, 30), 0, 30, 0, sH - 4);

      tft.fillRect(gX + 2, sY, sW - 4, sH, CL_DARK_GREY);
      tft.drawRect(gX, sY, sW, sH, CL_WHITE);
      ...
      tft.fillRect(
        ...
        sW - 4,
        fill,
        tdaMute ? warnCol : CL_YELLOW
      );

      tft.setCursor(gX, sY + sH + 10);
      tft.print("GAIN");

      tft.setCursor(gX, sY - 22);
      tft.print(tdaGain);
      tft.print("  ");
   ...
   int tX = sX + (sW + gap) * 4;
    ...
    // 6. BAL
    int balX = sX + (sW + gap) * 5;

    if (tdaBalance != l_balance || tdaMute != l_mute || blinkUpdate) {

      int fill = map(constrain(tdaBalance, -40, 40), -40, 40, 0, sH - 4);

      tft.fillRect(balX + 2, sY, sW - 4, sH, CL_DARK_GREY);
      tft.drawRect(balX, sY, sW, sH, CL_WHITE);

      uint16_t balCol = tdaMute ? warnCol : CL_BLUE;
      ...
      tft.setCursor(balX, sY + sH + 10);
      tft.print("BAL");

      tft.setCursor(balX, sY - 22);
      if (tdaBalance > 0) tft.print("+");
      tft.print(tdaBalance);
      tft.print("  ");

      l_balance = tdaBalance;
    }
    // 7. MUTE
    int muteX = 390;
    if (tdaMute != l_mute) {
      tft.fillRoundRect(380, 265, 90, 45, 8, tdaMute ? CL_RED : CL_GREY);
      tft.drawRoundRect(380, 265, 90, 45, 8, CL_WHITE);
      tft.setTextColor(CL_WHITE);
      tft.setCursor(400, 278);
      tft.print("MUTE");
      l_mute = tdaMute;
    }

    // 8. INPUT (4 realūs įėjimai, centruoti)
    if (tdaInput != l_input) {
...
void loop() {
  ...
  if (p.z > 100 && p.z < 2500) {
    ...
    if (px < sidebarWidth) {
     ...
     if (currentPage == 8) {
        int sX = sidebarWidth + 15;
        int sY = 70; int sH = 165; int sW = 40; int gap = 15;
        ...
        if (!tdaMute && py >= sY && py <= sY + sH) {
            if (px >= sX && px <= sX + sW) {
              ...
              tdaGain = map(constrain(py, sY, sY + sH), sY + sH, sY, 0, 30);
              setInputGain(tdaGain);
            }
            ...
            else if (px >= sX + (sW + gap) * 4 && px <= sX + (sW + gap) * 4 + sW) {
              tdaTreb = map(constrain(py, sY, sY + sH), sY + sH, sY, -15, 15);
              setTreble(tdaTreb);
            }
            else if (px >= sX + (sW + gap) * 5 && px <= sX + (sW + gap) * 5 + sW) {
              tdaBalance = map(constrain(py, sY, sY + sH), sY + sH, sY, -40, 40);

            if (tdaBalance > 0) {
               setBalance(0, tdaBalance);
            } else if (tdaBalance < 0) {
               setBalance(-tdaBalance, 0);
            } else {
               setBalance(0, 0);
            }
          }
        ...
        if (px >= 380 && px <= 470 && py >= 265 && py <= 310) {
             tdaMute = !tdaMute;
             setMute(tdaMute);
             l_mute = !tdaMute;
             delay(350);
